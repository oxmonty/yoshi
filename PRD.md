...
3. **Download size vs the field** (the cheapest headline) — macOS arm64 download of yoshi vs nteract Desktop, Zed, VS Code, and JupyterLab Desktop, fetched live from release APIs and rendered as a light/dark bar-chart pair by `script/bench-download-size` (matplotlib, dev-only dep). Regenerated per release; embedded in the README at E8 alongside the startup numbers. First measurement 2026-07-18: yoshi 4.2 MB vs 36/142/273/294 MB.
...
macOS: signed + notarized `.dmg` via GitHub Releases + Homebrew **cask** in `oxmonty/homebrew-tap` (`brew install oxmonty/tap/yoshi` — macOS-only, since Homebrew on Linux doesn't support casks; the `yoshi` CLI is exposed via the cask's `binary` stanza). Linux: AppImage on GitHub Releases (one format for MVP; `.deb` is future). Signing and notarization land at v0.1 (E8); before that, releases ship unsigned artifacts with the Gatekeeper bypass documented — modern macOS shows unsigned quarantined apps as "damaged" and right-click-Open no longer bypasses, so the documented paths are the quarantine strip (`xattr -dr com.apple.quarantine`) or the curl installer, whose downloads never acquire the quarantine flag. Release automation cuts all artifacts from one tag. No crates.io publication — yoshi is an app, and the name is taken there anyway.

**Community (E8):** a Discord server is the project's support and feedback channel, launched with v0.1 — invite link in the README and release notes. One channel only; no forum or Slack.

**Upgrade (E8):** `yoshi upgrade` (alias `update`) follows biscuit's two-channel model — `stable` and `next`, with alpha/beta/rc living in the version string, never as separate channels. Discovery is the GitHub Releases API: `/releases/latest` for stable (excludes prereleases by definition), plural `/releases` newest-first for `next`. Channel rules: `next` installs upgrade within `next`; stable never crosses onto prereleases; the one automatic crossing is graduation (a stable release newer than the latest prerelease pulls `next` users onto it). Replacement is install-channel-dispatched: brew-cask installs exec `brew upgrade --cask yoshi`; AppImage and bare `.app` installs self-download-and-swap, verifying against the release's `SHA256SUMS-*.txt` first (an improvement over biscuit, which skips verification). No background update checks — the command runs only when invoked.
...
**Versioning:** semver via release-please (active from E2), conventional commits. Pre-MVP releases are a `0.1.0-alpha.N` prerelease series, one release-please PR merged per epic exit, until E8 cuts `v0.1.0`. Breaking = anything that changes on-disk notebook output or settings schema. Known ceiling: release-please bumps `Cargo.toml` via a generic annotation, so `Cargo.lock`'s workspace-member versions lag one release behind until the next ordinary commit refreshes them — harmless while nothing builds `--locked`; switch to the `rust` release-type/cargo-workspace plugin if that ever changes.
...
- **Telemetry stance**: none in MVP; tier-2 adds *opt-in, local-only* logging of unrenderable MIME types to guide renderer priorities. Stated in README — it's a differentiator against Warp's own telemetry reputation. Adoption is measured server-side only (VLC's model): GitHub Releases `download_count` + Homebrew analytics, never an in-app ping.
...
- **Window chrome**: custom-drawn unified titlebar with inline traffic lights (hidden native titlebar via GPUI `TitlebarOptions`; Zed draws its chrome this way and is the architecture reference). Contents are strictly yoshi-specific and grow with the epics, with the exact controls spiked when each epic starts (E6 candidates: notebook name + dirty marker, kernel status + picker; E10: workspace controls like the tree toggle). Deliberately absent: a global search field and an update button (Warp's bar is the visual reference, not the feature list).
...
- **When does release automation land?** (**Resolved 2026-07-18: release-please from E2**, not manual tags until E8. First release forced to `0.1.0-alpha.1` via a `Release-As` footer; prerelease series with one release PR merged per epic exit. Rationale: the walking skeleton should exercise the real pipeline, and the bot PR proved unobtrusive against direct-push-to-main. Evidence: PR #1 → `v0.1.0-alpha.1` with five assets, smoke-tested from the artifacts. Folded into [CI/CD](#cicd).)
- **How do users upgrade an installed yoshi?** (**Resolved 2026-07-18, design only — built at E8**: `yoshi upgrade` following biscuit's two-channel model (`stable`/`next`, prerelease maturity in the version string), GitHub Releases discovery, checksum-verified self-swap for AppImage/bare `.app`, `brew upgrade --cask` delegation. No background update checks. Folded into [Distribution](#distribution).)
