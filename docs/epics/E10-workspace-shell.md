...
- [ ] Titlebar grows workspace controls, spiked at epic start — candidates: tree toggle leading, per-pane identity moving into pane headers, notebook tabs inline with the traffic lights (Warp's tab strip as UX reference) with right-click rename and color coding. Global search/command palette stays out (Future, with in-notebook `⌘F`)
