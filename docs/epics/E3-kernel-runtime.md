...
- [ ] Kernelspec discovery reads kernelspec JSON directly from disk (all standard dirs, including `~/Library/Jupyter/kernels` on macOS; never shells out to `jupyter`) — absorb the sync lister E2 left in `yoshi-cli` so discovery lives behind `yoshi-kernels`
