...
- [ ] Custom titlebar, minimal: hidden native titlebar with inline traffic lights (GPUI `TitlebarOptions`, Zed's mechanism — architecture reference only); exact bar contents spiked at epic start — candidates: notebook name + dirty marker, kernel status dot + picker. Contents stay yoshi-specific (no search field, no update button); E10 adds its workspace controls
